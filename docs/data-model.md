...
在目标年份专项公告尚未发布时，可使用节点顶层的 `holiday_reference` 保存上一年度官方安排。该对象必须使用 `status: historical_reference_only`，同时记录参考年份、适用日期、已观察事实、规划推断、来源、可信度和复核窗口。历史参考不得写入 `official_info`，也不得被 AI 表述为目标年份已经确认的开放、票价、预约或限流规则；目标年份公告发布后应以正式事实覆盖规划推断，但可保留历史记录用于趋势分析。
